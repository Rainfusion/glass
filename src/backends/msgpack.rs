...
pub fn objects_to_bytes<T>(objects: &Vec<(Uuid, T)>) -> Result<Vec<u8>, Box<Error>>
where
    T: serde::ser::Serialize,
{
...
pub fn bytes_to_object<'de, T>(bytes: &'de Vec<u8>) -> Result<(Uuid, T), Box<Error>>
where
...
pub fn bytes_to_objects<'de, T>(bytes: &'de Vec<u8>) -> Result<Vec<(Uuid, T)>, Box<Error>>
where
    T: serde::de::Deserialize<'de>,
{
    Ok(rmp_serde::from_slice(&bytes)?)
